...
#define DENS 2.2           // tau.rho
#define rad .55            // sphere radius
...
vec3 sunColor = 10.*vec3(0.412,0.412,0.412);   // NB: is Energy 
...
void main()
{
    ...
    vec3 cameraTarget = vec3(0.0, -0.05, 0.0);
    ...
    //light
    //float theta = (0 / iResolution.x *2. - 1.)*PI;
    //float phi = (0 / iResolution.y - .5)*PI;
    vec3 lightDir =vec3(1.0,0,1.0);
  
	// shade object
    float transmittance = computeTransmittance( cameraPos, cameraDir, lightDir );
	
	// display: special cases
	//if (abs(q.y-.42)*iResolution.y<.5)  fragColor = vec4(1,1,1,0);
    //else if (transmittance<0.) 		    fragColor = vec4(1,1,1,0);
